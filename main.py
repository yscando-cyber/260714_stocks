...
# 페이지 설정
# ----------------------------------------------------------------
st.set_page_config(page_title="📈 주식 데이터 분석 대시보드", page_icon="📈", layout="wide")
...
ticker_input = st.sidebar.text_input(
    "티커 입력 (쉼표로 구분, 예: AAPL, TSLA, 005930.KS)",
    value="AAPL"
).upper()
...
if df.empty:
    st.error(f"'{main_ticker}'에 대한 데이터를 불러올 수 없습니다. 티커를 확인해주세요.")
    st.stop()
...
col1.metric("현재가", f"{last_close:,.2f}", f"{change:,.2f} ({change_pct:.2f}%)")
col2.metric("최고가", f"{float(df['High'].max()):,.2f}")
col3.metric("최저가", f"{float(df['Low'].min()):,.2f}")
...
fig = make_subplots(
    ...
    subplot_titles=(f"{main_ticker} 가격", "거래량")
)
...
if len(tickers) > 1:
    ...
    for t in tickers:
        ...
        comp_fig.add_trace(go.Scatter(x=t_df.index, y=normalized, mode="lines", name=t))
